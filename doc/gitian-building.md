...
Gitian is the deterministic build process that is used to build the ADCOIN
...
- [Building ADCOIN](#building-adcoin)
...
Clone the git repositories for adcoin and gitian and then checkout the adcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/adcoin-crypto/adcoin.git
cd adcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the adcoin repository
...
To build ADCOIN (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the adcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/adcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/adcoin-crypto/adcoin
...
For example:
```bash
URL=https://github.com/crowning-/adcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit adcoin=${COMMIT} --url adcoin=${URL} ../adcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit adcoin=${COMMIT} --url adcoin=${URL} ../adcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit adcoin=${COMMIT} --url adcoin=${URL} ../adcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/adcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/adcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/adcoin-build.assert
```
...
[adcoin/gitian.sigs](https://github.com/adcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.

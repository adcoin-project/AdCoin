...
CzADCOINWallet::CzADCOINWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zadcoin seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZADCOINSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZADCOINSeed_deprecated()) {
                    LogPrintf("%s: Updated zADCOIN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zadcoin seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zadcoin wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzADCOINWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZADCOINCount(nCountLastUsed);
    else if (!walletdb.ReadZADCOINCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzADCOINWallet::Lock()
{
    seedMaster = 0;
}

void CzADCOINWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzADCOINWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozADCOIN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzADCOINWallet::LoadMintPoolFromDB()
{
...
void CzADCOINWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzADCOINWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzADCOINWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zadcoinTracker->HasPubcoinHash(pMint.first)) {
...
bool CzADCOINWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozADCOIN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zadcoinTracker which also adds to database
    pwalletMain->zadcoinTracker->Add(dMint, true);
...
void CzADCOINWallet::SeedTozADCOIN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzADCOINWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzADCOINWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZADCOINCount(nCountLastUsed);
...
void CzADCOINWallet::GenerateDeterministiczADCOIN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzADCOINWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzADCOINWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

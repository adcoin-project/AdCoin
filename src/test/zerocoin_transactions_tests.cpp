...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zadcoinTracker = unique_ptr<CzADCOINTracker>(new CzADCOINTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZADCOIN_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZADCOIN_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");

...
int nAnonymizeADCOINAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "adcoin" is a composite category enabling all ADCOIN-related debug output
            if (ptrCategory->count(string("adcoin"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "adcoin.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty adcoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override adcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "adcoind.pid"));

...
#ifndef ADCOIN_zADCOINWALLET_H
#define ADCOIN_zADCOINWALLET_H
...
public:
    CzADCOINWallet(std::string strWalletFile);
    ...
    void GenerateDeterministiczADCOIN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedTozADCOIN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ADCOIN_zADCOINWALLET_H

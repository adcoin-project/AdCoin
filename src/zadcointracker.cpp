...
CzADCOINTracker::CzADCOINTracker(std::string strWalletFile)
{
...
CzADCOINTracker::~CzADCOINTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzADCOINTracker::Init()
{
...
bool CzADCOINTracker::Archive(CMintMeta& meta)
{
...
bool CzADCOINTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzADCOINTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzADCOINTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzADCOINTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzADCOINTracker::GetSerialHashes()
{
...
CAmount CzADCOINTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzADCOINTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzADCOINTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzADCOINTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzADCOINTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzADCOINTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzADCOINTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzADCOINTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzADCOINTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzADCOINTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzADCOINTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzADCOINTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzADCOINTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzADCOINTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzADCOINTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzADCOINTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzADCOINTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzadcoin from DB\n", __func__, listDeterministicDB.size());
    }

...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZADCOINStake(spend));
    } else {
        ...
        CADCOINStake* adcoinInput = new CADCOINStake();
        adcoinInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(adcoinInput);
    }

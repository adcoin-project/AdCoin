...
#include "zadcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zADCOIN ought to be enough for anybody." - Bill Gates, 2017
    ui->zADCOINpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzADCOINSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zADCOINpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzADCOIN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADCOIN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zADCOIN, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zADCOIN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zADCOIN in ") +
...
void PrivacyDialog::on_pushButtonSpendzADCOIN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADCOIN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zADCOIN, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zADCOIN
        sendzADCOIN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zADCOIN
    sendzADCOIN();
}

void PrivacyDialog::on_pushButtonZADCOINControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZADCOINControlDialog* zADCOINControl = new ZADCOINControlDialog(this);
    zADCOINControl->setModel(walletModel);
    zADCOINControl->exec();
}

void PrivacyDialog::setZADCOINControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzADCOINSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzADCOIN()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ADCOIN Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zADCOINpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zADCOIN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ADCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zADCOIN</b>";
    ...
    // use mints from zADCOIN selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZADCOINControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZADCOINControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zADCOIN require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zADCOIN"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZADCOIN_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zADCOIN require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zADCOIN"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zADCOIN was spent successfully update the addressbook with the label
    ...
    // Clear zadcoin selector in case it was used
    ZADCOINControlDialog::setSelectedMints.clear();
    ui->labelzADCOINSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zADCOIN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ADCOIN, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zADCOIN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zadcoinTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zADCOIN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zADCOIN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zADCOIN "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zADCOIN "));
    ui->labelzADCOINAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zADCOIN </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zADCOIN </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zADCOIN </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzADCOINSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzADCOIN->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zADCOIN
        ui->pushButtonMintzADCOIN->setEnabled(false);
        ui->pushButtonMintzADCOIN->setToolTip(tr("zADCOIN is currently disabled due to maintenance."));

        // Spend zADCOIN
        ui->pushButtonSpendzADCOIN->setEnabled(false);
        ui->pushButtonSpendzADCOIN->setToolTip(tr("zADCOIN is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zADCOIN
        ui->pushButtonMintzADCOIN->setEnabled(true);
        ui->pushButtonMintzADCOIN->setToolTip(tr("PrivacyDialog", "Enter an amount of ADCOIN to convert to zADCOIN", 0));

        // Spend zADCOIN
        ui->pushButtonSpendzADCOIN->setEnabled(true);
        ui->pushButtonSpendzADCOIN->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

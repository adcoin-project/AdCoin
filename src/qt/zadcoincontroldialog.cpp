...
#include "ui_zadcoincontroldialog.h"
...
std::set<std::string> ZADCOINControlDialog::setSelectedMints;
std::set<CMintMeta> ZADCOINControlDialog::setMints;

bool CZADCOINControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZADCOINControlDialog::COLUMN_DENOMINATION || column == ZADCOINControlDialog::COLUMN_VERSION || column == ZADCOINControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZADCOINControlDialog::ZADCOINControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZADCOINControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZADCOINControlDialog::~ZADCOINControlDialog()
{
    delete ui;
}

void ZADCOINControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZADCOINControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZADCOINControlWidgetItem* itemDenom(new CZADCOINControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZADCOINControlWidgetItem *itemMint = new CZADCOINControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZADCOINControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZADCOINControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZADCOIN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZADCOINControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZADCOINControlDialog::GetSelectedMints()
{
...
void ZADCOINControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

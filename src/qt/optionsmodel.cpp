...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ADCOIN);
    ...
    if (!settings.contains("nAnonymizeADCOINAmount"))
        settings.setValue("nAnonymizeADCOINAmount", 1000);

    nAnonymizeADCOINAmount = settings.value("nAnonymizeADCOINAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeADCOINAmount"))
        SoftSetArg("-anonymizeadcoinamount", settings.value("nAnonymizeADCOINAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in adcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeADCOINAmount:
            return QVariant(nAnonymizeADCOINAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeADCOINAmount:
            nAnonymizeADCOINAmount = value.toInt();
            settings.setValue("nAnonymizeADCOINAmount", nAnonymizeADCOINAmount);
            emit anonymizeADCOINAmountChanged(nAnonymizeADCOINAmount);
            break;
        case CoinControlFeatures:
